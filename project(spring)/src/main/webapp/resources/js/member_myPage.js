$(document).ready(function(){
	...
	var oldPwfalse = "현재 비밀번호 불일치";
	var oldPwtrue = "현재 비밀번호 일치";
	$("#mPasswd").keyup(function(){
		...
		// 현재 비밀번호 불일치
		if ($("#result1").text()==oldPwfalse){
			$("#result2").text("현재 비밀번호가 틀립니다.").css("color","red");
			oldPw.focus();
			return false;
		} else if ($("#result1").text()==oldPwtrue) {
			mPasswd.focus();
		}
		...
		//새 비밀번호 일치 확인
		if( mPasswd.val() == mPasswd2.val() && mPasswd.length > 1) {
			$("#result2").text("비밀번호가 일치 합니다.").css("color","blue");
		} else if( $(mPasswd).val().length==1 && $(mPasswd2).val().length==1 ){
			$("#result2").text("");
			return false;
		}
	...
	$("#mPasswd2").keyup(function(){
		...
		if( mPasswd.val() == mPasswd2.val() && $(mPasswd).val().length > 1 ) {
			$("#result2").text("비밀번호가 일치 합니다.").css("color","blue");
		} else if( $(mPasswd).val().length==1 && $(mPasswd2).val().length==1 ){
			$("#result2").text("");
			return false;
	...
	$("#mPwUpdate").submit(function(){
		...
		if($("#result1").text()=="" || $("#result1").text()==oldPwfalse){
	...
	$("#mPhoneUpdateForm").submit(function(){
		...
		if(re_phone.test(phone2)!= true){
			alert('유효한 전화번호를 입력해주세요.');
			$("#mPhone2").focus();
			return false;
		}else if(re_phone.test(phone3)!= true){
			alert('유효한 전화번호를 입력해주세요.');
			$("#mPhone3").focus();
			return false;
